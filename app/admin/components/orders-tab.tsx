...
const statusConfig = {
    ...
    preparing: { label: 'Em Preparo', bg: 'bg-yellow-800', text: 'text-white' },
...
const orderCardColors = {
    ...
    preparing: 'bg-yellow-700 text-white',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.keys(statusConfig).map(status => {
                ...
                return (
                    <GlassCard
                        key={status}
                        className={`p-3 rounded-lg border border-gray-700 shadow-md ${statusConfig[status as keyof typeof statusConfig].bg} ${statusConfig[status as keyof typeof statusConfig].text}`}
                    >
                        <h3 className="font-semibold mb-3 text-md border-b border-gray-600 pb-1">
                            {statusConfig[status as keyof typeof statusConfig].label}
                        </h3>
                        <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
                            {ordersByStatus.length === 0 && <p className="text-sm text-gray-300 italic">Nenhum pedido</p>}
                            {ordersByStatus.map(order => (
                                <GlassCard
                                    key={order.id}
                                    className={`p-3 rounded-md border border-gray-600 shadow hover:shadow-lg cursor-pointer transition ${orderCardColors[order.status as keyof typeof orderCardColors]}`}
                                ...
                                >
                                    ...
                                    <ul className="mb-1 list-disc list-inside text-xs">
                                        {order.order_items.map(item => (
                                            <li key={item.id}>{item.product?.name} x{item.quantity} - {formatPrice(item.unit_price_cents)}</li>
                                        ))}
                                    </ul>
